...
#使用pymysql进行数据库连接
conn = pymysql.connect(host="127.0.0.1",port=3306,user='root',passwd='hdu220313',db='nba',charset='utf8')
cur = conn.cursor()

#新闻存储函数
def storeNews(dbPubtime,dbLink,dbTitle,dbContent):
    cur.execute("SELECT id FROM sohu WHERE link=%s",dbLink)
    checkExist = cur.fetchone()
    if checkExist == None:
        cur.execute("INSERT INTO sohu(source,pubtime,link,title,content) VALUES(%s,%s,%s,%s,%s)",("搜狐",dbPubtime,dbLink,dbTitle,dbContent))
...
for n in snewsList:
    ...
    scontent_out = re.sub('[\n\r\t\xa0\u200b]', '', scontent)
    #首页去重
    storeNews(spubtime, sinUrl, stitle, scontent_out)
...
def pageCollect(url):
    ...
    for n in newsList:
        ...
        content_out = re.sub('[\n\r\t\xa0\u200b]', '', content)
        storeNews(pubtime, inUrl, title, content_out)
